...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //                |          |  |                 |
        //                V          V  V                 V
        glDrawElements(GL_TRIANGLE_FAN, POINTS_NUMBER, GL_UNSIGNED_BYTE, 0);
...
GLuint BuildTriangles()
{
    ...
    NDC_coefficients[3] = RADIUS;

    float angle = 2*PI;
    float angle_triangle = (2*PI)/(POINTS_NUMBER-2);

    for(i = 4; i <= (POINTS_NUMBER+2) * 4; i += 4){
        NDC_coefficients[i + 0] = (float)(RADIUS * cos(angle));
        NDC_coefficients[i + 1] = (float)(RADIUS * sin(angle));
        NDC_coefficients[i + 3] = 1.0f;
        angle = angle - angle_triangle;
    }
