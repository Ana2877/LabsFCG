...
const float RADIUS_INTERNAL = 0.5f;
const float RADIUS_EXTERNAL = 0.7f;
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //                |          |  |                 |
        //                V          V  V                 V
        glDrawElements(GL_TRIANGLE_STRIP, POINTS_NUMBER, GL_UNSIGNED_BYTE, 0);
...
GLuint BuildTriangles()
{
    ...
    for(i = 4; i <= POINTS_NUMBER * 4; i += 8){
        NDC_coefficients[i + 0] = (float)(RADIUS_EXTERNAL * cos(angle));
        NDC_coefficients[i + 1] = (float)(RADIUS_EXTERNAL * sin(angle));
        NDC_coefficients[i + 3] = 1.0f;
        NDC_coefficients[i + 4] = (float)(RADIUS_INTERNAL * cos(angle));
        NDC_coefficients[i + 5] = (float)(RADIUS_INTERNAL * sin(angle));
