...
const int POINTS_NUMBER = 36;
const float RADIUS_INTERNAL_Y = -0.1f;
const float RADIUS_EXTERNAL_Y = -0.2f;
const float RADIUS_INTERNAL_X = 0.30f;
const float RADIUS_EXTERNAL_X = 0.4f;

// Declaração de várias funções utilizadas em main().  Essas estão definidas
// logo após a definição de main() neste arquivo.
GLuint NumberOne(float spaceOffset);
GLuint NumberZero(float spaceOffset);
...
int* contador_binario(int binario[4]);
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(500, 500, "INF01047 - 00287714 - Ana Carolina Pagnoncelli", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    GLuint VAO_one3=NumberOne(-0.75);
    GLuint VAO_one2=NumberOne(-0.25);
    GLuint VAO_one1=NumberOne(0.25);
    GLuint VAO_one0=NumberOne(0.75);

    GLuint VAO_zero0=NumberZero(-0.75);
    GLuint VAO_zero1=NumberZero(-0.25);
    GLuint VAO_zero2=NumberZero(0.25);
    GLuint VAO_zero3=NumberZero(0.75);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if 	(binario[1]== 0)
        {
            glBindVertexArray(VAO_zero0);
            glDrawElements(GL_TRIANGLE_STRIP, 36, GL_UNSIGNED_BYTE, 0);
            glBindVertexArray(0);
        }
        else
        {
            glBindVertexArray(VAO_one0);
            glDrawElements(GL_TRIANGLE_STRIP, 9, GL_UNSIGNED_BYTE, 0);
            glBindVertexArray(0);
        }
        if 	(binario[2]== 0)
        {
            glBindVertexArray(VAO_zero1);
            glDrawElements(GL_TRIANGLE_STRIP, 36, GL_UNSIGNED_BYTE, 0);
            glBindVertexArray(0);
        }
        else
        {
            glBindVertexArray(VAO_one1);
            glDrawElements(GL_TRIANGLE_STRIP, 9, GL_UNSIGNED_BYTE, 0);
            glBindVertexArray(0);
        }
        if 	(binario[3]== 0)
        {
            glBindVertexArray(VAO_zero2);
            glDrawElements(GL_TRIANGLE_STRIP, 36, GL_UNSIGNED_BYTE, 0);
            glBindVertexArray(0);
        }
        else
        {
            glBindVertexArray(VAO_one2);
            glDrawElements(GL_TRIANGLE_STRIP, 9, GL_UNSIGNED_BYTE, 0);
            glBindVertexArray(0);
        }
        if 	(binario[4]== 0)
        {
            glBindVertexArray(VAO_zero3);
            glDrawElements(GL_TRIANGLE_STRIP, 36, GL_UNSIGNED_BYTE, 0);
            glBindVertexArray(0);
        }
        else
        {
            glBindVertexArray(VAO_one3);
            glDrawElements(GL_TRIANGLE_STRIP, 9, GL_UNSIGNED_BYTE, 0);
            glBindVertexArray(0);
        }
...
int* contador_binario(int binario[4])
{
        ...
        //converte os segundos para binario e atribui à um array
        for(cont=1; cont <= 4; cont++)
        {
            resto = segundos%2;
            binario[cont] = resto;
...
GLuint NumberOne(float spaceOffset)
{
    ...
    //    X      Y     Z     W
        -0.15f+spaceOffset, 0.15f, 0.0f, 1.0f,
        -0.05f+spaceOffset, 0.25f, 0.0f, 1.0f,
        -0.05f+spaceOffset, 0.35f, 0.0f, 1.0f,
         0.05f+spaceOffset, 0.35f, 0.0f, 1.0f,
        -0.05f+spaceOffset, -0.35f, 0.0f, 1.0f,
         0.05f+spaceOffset, -0.35f, 0.0f, 1.0f
    };
    ...
    //
    GLubyte indices[] = { 0,1,2, 2,4,3, 3,4,5}; // GLubyte: valores entre 0 e 255 (8 bits sem sinal).
...
GLuint NumberZero(float spaceOffset)
{
    ...
    // Este vetor "NDC_coefficients" define a GEOMETRIA (veja slides 64-71 do documento Aula_04_Modelagem_Geometrica_3D.pdf).
    //
    GLfloat NDC_coefficients[POINTS_NUMBER*4] = {0.0f};

    int i;
    double theta;

    NDC_coefficients[3] = RADIUS_EXTERNAL_Y;

    float angle = 2*PI;
    float angle_triangle = (2*PI)/(POINTS_NUMBER-4);

    for(i = 4; i <= POINTS_NUMBER * 4; i += 8){
        NDC_coefficients[i + 0] = (float)(RADIUS_EXTERNAL_Y * cos(angle)) - spaceOffset;
        NDC_coefficients[i + 1] = (float)(RADIUS_EXTERNAL_X * sin(angle));
        NDC_coefficients[i + 3] = 1.0f;
        NDC_coefficients[i + 4] = (float)(RADIUS_INTERNAL_Y * cos(angle))- spaceOffset;
        NDC_coefficients[i + 5] = (float)(RADIUS_INTERNAL_X * sin(angle));
        NDC_coefficients[i + 7] = 1.0f;
        angle = angle - 2*angle_triangle;
    }
    ...
    // Este vetor "indices" define a TOPOLOGIA (veja slides 64-71 do documento Aula_04_Modelagem_Geometrica_3D.pdf).
    //
    GLubyte indices[POINTS_NUMBER]; // GLubyte: valores entre 0 e 255 (8 bits sem sinal).
